...
class AGCNEncoder(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, in_channels=3, adaptive=True, attention=True,ATU_layer=2): #change 3 to 64
...
class AAGCN_VA_RICH(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=15,
        ...
        super(AAGCN_VA_RICH, self).__init__()
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 4, 3, 1, 2).contiguous().view(N, M * V * C, T)  # (16, 750, 300)


        x = self.data_bn(x.cuda())
        x = x.view(N, M, V, C, T).permute(0, 1, 3, 4, 2).contiguous().view(N * M, C, T, V)  # (32, 15, 300, 25)
        ...
        # N*M,C,T,V
        c_new = x.size(1)
        x = x.view(N, M, c_new, -1) #16 2 256 1875
        # print('x={}'.format(x.shape))

        x = x.mean(3).mean(1) #16 256
        # print(x.shape)
