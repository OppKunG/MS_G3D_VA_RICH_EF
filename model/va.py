...
class ViewAdaptive(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('trans_reshape', trans_reshape.shape)
        ...
        # print('trans_finally', trans.shape)
        ...
        # print('translation', translation[0, 0:10, :])
...
def VA(x):
    ...
    ### For translation
    trans = _trans(theta)
    output = _transform_trans(trans, conv_input)  # number 2 translation

    ### Calculate centroid
    cx = torch.mean(output.view(-1, output.size(1) * output.size(2))[:, 0::3], dim=1)
    cy = torch.mean(output.view(-1, output.size(1) * output.size(2))[:, 1::3], dim=1)
    cz = torch.mean(output.view(-1, output.size(1) * output.size(2))[:, 2::3], dim=1)
    output = _translate_by_centroid(output, cx, cy, cz, d=-1)  # number 3 translation

    ### For rotation
    M = _fusion(theta)
    output = _transform_rot(M, output, output.size()[0])  # number 3 rotation

    ### Translate back
    output = _translate_by_centroid(output, cx, cy, cz, d=1)  # number 4 translation
    # print(output.shape)
    return output
...
def _trans(theta):
    ...
    # print('zero', zero.shape #torch.Size([9600, 1])
    ...
    trans = torch.cat([first, second, third], dim=1)
    # print('trans', trans.shape) #torch.Size([9600, 3, 4])
...
def _transform_rot(theta, input, batch_shape):
    ...
    input = input.permute(0, 2, 1)
    add = torch.ones(batch * step, 1, dim // 3).to(device)
    input = torch.cat((input, add), dim=1)  # why

    output = torch.bmm(theta, input)
    output = output.permute(0, 2, 1)
    output = output[:, :, 0:3]  # why
