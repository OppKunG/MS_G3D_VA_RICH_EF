...
class Decoder(nn.Module):
    def __init__(self):
        ...
        # self.l3 = nn.Linear(64,3) #change

    def forward(self,x):
        N, M,C,T,V = x.size()
        x = x.reshape(N*M,C,T,V)
        x = x.view(N,M,128,T,V).permute(0,1,3,4,2)
        # x = x.view(N,M,64,T,V).permute(0,1,3,4,2) #change
        x = self.l3(x)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,self_supervised_mask=1,if_rotation=False,seg_num=1,if_vibrate=False,prediction_mask=0,GCNEncoder="AGCN",ATU_layer=2,T=300,predict_seg=1):
        super(Model, self).__init__()
        # print('in_channels: ',in_channels) #3

        self.lefthand = shiftGCNEncoder.ShiftGCNEncoder(num_class, 6, num_person, graph, graph_args,in_channels) #(60, 6, 2, None, {}, 3)
        ...
        if GCNEncoder == "CTRGCN":
            ...
            self.Encoder = AGCNEncoder.AGCNEncoder(num_class, num_point, num_person, graph, graph_args,64) #change
        elif GCNEncoder == "shiftGCN":
            self.Encoder = shiftGCNEncoder.ShiftGCNEncoder(num_class, num_point, num_person, graph, graph_args,64)
            # self.Encoder = shiftGCNEncoder.ShiftGCNEncoder(num_class, num_point, num_person, graph, graph_args,in_channels) #change
        ...
        # self.JointDirectionClassifier = JointDirectionClassifier() #change
        self.JointDirectionClassifier = Classifier()
    ...
    def forward(self,x,is_test=False):
        ...
        if is_test == False:
            ...
            # print('predict_seg_length: ',predict_seg_length) #100
            # print('x.shape: ',x.shape) #torch.Size([32, 3, 100, 25, 2])
            
            for i in range(1,self.predict_seg+1):
                # print(int(i*predict_seg_length-prediction_length),int(i*predict_seg_length))
        ...
        # GCN_feature = torch.empty(N,M,3,T,V).cuda().to(torch.float32) #change 64 to 3 [16, 2, 3, 300, 25]
        
        # 將各部分特徵分別賦值給GCN_feature

        GCN_feature[:,:,:,:,self.LEFT_HAND-1] = self.lefthand(x[:,:,:,self.LEFT_HAND-1,:]) #torch.Size([16, 2, 64, 300, 6])
